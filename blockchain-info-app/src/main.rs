...
fn blockchain_info_app(address: &str) {
    ...
    if command.trim().eq("y") {
        ...
        for tx_id in &blockchain_address.txids {
            ...
            // On the BTC blockchain vout is used to represent money coming in while vin is used to represent money going out
...
fn main() {
    let entered_address = dotenv::var("WALLET").expect("Error reading env var: WALLET");
